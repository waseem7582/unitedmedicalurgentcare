...
class PaymentGatewaysController extends Controller
{
    ...
    public function automaticPaymentMethodView() {
        $page_title = __("Automatic Payment Method");
    ...
    public function automaticPaymentMethodEdit($alias) {
        $page_title = __("Automatic Payment Method Edit");
    ...
    public function automaticPaymentMethodStore(Request $request) {
        ...
        $validator = Validator::make($request->all(),[
            'gateway_name'              => ['required','string','max:60',Rule::unique('payment_gateways','alias')->where(function($query) use ($gateway_name) {
                ...
                $query->where('slug',PaymentGatewayConst::payment_gateway_slug())->where('type',PaymentGatewayConst::AUTOMATIC)->where('alias',$alias);
            })],
    ...
    public function paymentGatewayStatusUpdate(Request $request) {
        ...
        if(!$payment_gateway) {
            $error = ['error' => [__('Payment gateway not found!.')]];
    ...
    /**
     * Function for Automatic Payment Method Update Based on alias
     * @param \Illuminate\Http\Request $request
     * @param string $alias
     */
    public function automaticPaymentMethodUpdate(Request $request,$alias) {
        ...
        foreach($validated[$input_field_base_name] ?? [] as $currency => $item) {
            ...
            $item['alias']              = PaymentGatewayConst::payment_gateway_slug() . "-" . Str::slug($gateway->name . " " . $currency . " " . $gateway->type);
    ...
    public function manualAddMoneyStore(Request $request) {
        ...
        return $validated['input_fields']      = decorate_input_fields($validated);
    ...
    public function manualMoneyOutUpdate(Request $request,$alias) {
        ...
        return redirect()->route('admin.payment.gateway.view',['money-out','manual'])->with(['success' => [__('Payment gateway updated successfully!')]]);
    }
}

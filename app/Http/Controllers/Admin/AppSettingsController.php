...
class AppSettingsController extends Controller
{
    ...
    public function splashScreenUpdate(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'hospital_image'         => 'nullable|image|mimes:png,jpg,jpeg,webp,svg',
            'hospital_version'       => 'required|string|max:15',
        ]);
        ...
        if($request->hasFile('hospital_image')) {
            $image = get_files_from_fileholder($request,'hospital_image');
            $upload_image = upload_files_from_path_static($image,'splash-images',checkSeederValue($app_settings->hospital_splash_screen_image),true,true);
            $validated['hospital_splash_screen_image']   = $upload_image;
        }

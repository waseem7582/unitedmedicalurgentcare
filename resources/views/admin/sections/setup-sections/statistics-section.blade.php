...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#statistics-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i>
                        {{ __('Add Item') }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td> {{ $item->language->$system_default_lang->item_counter_value ?? '' }} </td>
                                <td>
    ...
    @include('admin.components.modals.site-section.statistics-section.add')

    @include('admin.components.modals.site-section.statistics-section.edit')
@endsection
@push('script')
    <script>
        ...
        $('.edit-modal-button').click(function() {
            ...
            editModal.find("input[name=" + default_language + "_item_counter_value_edit]").val(oldData.language[
                default_language].item_counter_value);
            editModal.find("input[name=" + default_language + "_item_title_edit]").val(oldData.language[
                default_language].item_title);
            editModal.find("input[name=" + default_language + "_item_description_edit]").val(oldData.language[
            ...
            $.each(languages, function(index, item) {
                editModal.find("input[name=" + item.code + "_item_title_edit]").val((oldData.language[item
                    .code] == undefined) ? '' : oldData.language[item.code].item_title);
                editModal.find("input[name=" + item.code + "_item_description_edit]").val((oldData.language[
                    item.code] == undefined) ? '' : oldData.language[item.code].item_description);
                editModal.find("input[name=" + item.code + "_item_counter_value_edit]").val((oldData
                        .language[item.code] == undefined) ? '' : oldData.language[item.code]
                    .item_counter_value);
            });

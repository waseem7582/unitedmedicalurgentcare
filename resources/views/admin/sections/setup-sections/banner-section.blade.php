        ...
        <div class="card-body">
            <form class="card-form" action="{{ setRoute('admin.setup.sections.section.update', $slug) }}" method="POST"
                ...
                <div class="row justify-content-center mb-10-none">
                    ...
                    <div class="col-xl-4 col-lg-4 form-group">
                        @include('admin.components.form.input-file', [
                            'label' => 'Secondary Image:' . '(531*213)',
                            'name' => 'secondary_image',
                            'class' => 'file-holder',
                            'old_files_path' => files_asset_path('site-section'),
                            'old_files' => $data->value->secondary_image ?? '',
                        ])
                    </div>
                                ...
                                @foreach ($languages as $item)
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif"
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                'label' => 'Left Button*',
                                                'name' => $item->code . '_left_button',
                                                'value' => old(
                                                    $item->code . '_left_button',
                                                    $data->value->language->$lang_code->left_button ?? ''),
                                            ])
                                        </div>
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                'label' => 'Right Button*',
                                                'name' => $item->code . '_right_button',
                                                'value' => old(
                                                    $item->code . '_right_button',
                                                    $data->value->language->$lang_code->right_button ?? ''),
                                            ])
                                        </div>

        ...
        <div class="table-wrapper">
            <div class="table-header-title">
                <div class="table-name mb-10">
                    <h3 class="title">{{ __("Hospital Branch") }}</h3>
                </div>
                <div class="add-btn text-end mb-10">
                    <a href="{{ setRoute('hospitals.branch.create') }}" class="btn--base"><i
                            class="fas la-plus"></i> {{ __('Add Branch') }}</a>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table table-striped custom-table">
                    ...
                    <tbody>
                        @forelse ($branch ?? [] as $key => $item)
                            <tr data-item="{{ $item }}">

                                <td>{{ $item->name ?? '' }}</td>
                                <td> {{ implode(', ', $item->departments->pluck('name')->toArray()) }}
                                </td>
                                <td>
                                ...
                                <td class="status-btn">
                                    <a href="{{ setRoute('hospitals.branch.edit', $item->uuid) }}" class="btn-edit">
...
@push('script')
    <script>
        $('.delete-btn').click(function() {
            ...
            var actionRoute = "{{ setRoute('hospitals.branch.delete') }}";
            var target = oldData.id;
            var message =
                `{{ __('Are you sure to') }} <strong>{{ __('delete') }}</strong> {{ __('this Branch?') }}`;
        ...
        // Switcher
        switcherAjax("{{ route('hospitals.branch.status.update') }}");
    </script>
@endpush

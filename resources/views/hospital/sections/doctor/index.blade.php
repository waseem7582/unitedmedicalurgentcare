        ...
        <div class="table-wrapper">
            <div class="table-header-title">
                <div class="table-name mb-10">
                    <h3 class="title">{{ __('Doctor List') }}</h3>
                </div>
                <div class="add-btn text-end mb-10">
                    <a href="{{ setRoute('hospitals.doctor.create') }}" class="btn--base"><i class="fas la-plus"></i>
                        {{ __('Add Doctor') }}</a>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table table-striped custom-table">
                    ...
                    <tbody>
                        @forelse ($doctor ?? [] as $key => $item)
                            <tr data-item="{{ $item }}">
                                <td>
                                    <div class="dr-profile">
                                        <img src="{{ get_image($item->image ?? '', 'doctor') ?? '' }}" alt="doctor">
                                    </div>
                                </td>
                                <td>{{ $item->name ?? '' }}</td>
                                <td>
                                ...
                                <td class="status-btn">
                                    <a href="{{ setRoute('hospitals.doctor.edit', $item->slug) }}" class="btn-edit">
...
@push('script')
    <script>
        $('.delete-btn').click(function() {
            ...
            var actionRoute = "{{ setRoute('hospitals.doctor.delete') }}";
            var target = oldData.id;
            var message =
                `{{ __('Are you sure to') }} <strong>{{ __('delete') }}</strong> {{ __('this Doctor?') }}`;
        ...
        // Switcher
        switcherAjax("{{ route('hospitals.doctor.status.update') }}");
    </script>
@endpush

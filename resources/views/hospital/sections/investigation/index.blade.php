        ...
        <div class="table-wrapper">
            <div class="table-header-title">
                <div class="table-name mb-10">
                    <h3 class="title">{{ __('Hospital Investigation') }}</h3>
                </div>
                <div class="add-btn text-end mb-10">
                    <a href="{{ setRoute('hospitals.investigation.create') }}" class="btn--base"><i class="fas la-plus"></i>
                        {{ __('Add Investigation') }}</a>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table table-striped custom-table">
                    ...
                    <tbody>
                        @forelse ($investigation ?? [] as $key => $item)
                            <tr data-item="{{ $item }}">
                                ...
                                <td> {{ implode(', ', $item->investigationCategory->pluck('name')->toArray()) }}
                                </td>
                                <td class="status-btn">
                                    <a href="{{ setRoute('hospitals.investigation.edit', $item->uuid) }}" class="btn-edit">
...
@push('script')
    <script>
        $('.delete-btn').click(function() {
            ...
            var actionRoute = "{{ setRoute('hospitals.investigation.delete') }}";
            var target = oldData.id;
            var message =
                `{{ __('Are you sure to') }} <strong>{{ __('delete') }}</strong> {{ __('this Investigation?') }}`;
        ...
        // Switcher
        switcherAjax("{{ route('hospitals.investigation.status.update') }}");
    </script>
@endpush
